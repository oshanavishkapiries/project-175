...
async function runColomboScraper() {
    console.log('🚀 Starting Colombo Restaurants Scraper...');
    ...
    try {
        ...
        // Search for "Colombo Restaurants"
        const url = 'https://www.google.com/maps/search/colombo+Restaurants/@6.901844,79.8550186,14z';
        ...
        // Loop to scroll
        // Tune this: higher loops = more restaurants
        ...
        console.log(`🔎 Found ${uniqueLinks.length} restaurants.`);

        const results = [];

        // 4. Extract Details
        for (let i = 0; i < uniqueLinks.length; i++) {
            const restaurantUrl = uniqueLinks[i];
            console.log(`[${i + 1}/${uniqueLinks.length}] Processing...`);

            try {
                await page.goto(restaurantUrl, { waitUntil: 'domcontentloaded' });
                ...
                console.error(`   ❌ Failed on ${restaurantUrl}:`, err.message);
            }
        ...
        if (results.length > 0) {
            ...
            const outPath = path.join(__dirname, '../../../data/colombo_restaurants.csv');
